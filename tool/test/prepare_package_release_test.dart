...
void main() {
  ...
  setUp(() {
    tempRoot = Directory.systemTemp.createTempSync('foundry_prepare_release_');
  });
...
''');
  ...
  group('validateTagFormat', () {
    test('accepts foundry monorepo format', () {
  ...
  group('renderTagFormat', () {
    test('renders foundry monorepo tag', () {
      expect(
        renderTagFormat(
          format: '{name}/{version}',
          name: 'foundry_core',
  ...
  group('tagGlobForFormat', () {
    test('builds foundry monorepo glob', () {
      expect(
        tagGlobForFormat(format: '{name}/{version}', name: 'foundry_core'),
        'foundry_core/*',
      );
    });
  ...
  group('parseVersionFromTag', () {
    test('parses foundry monorepo tag', () {
      expect(
        parseVersionFromTag(
          tag: 'foundry_core/0.0.1-dev.2',
          format: '{name}/{version}',
          name: 'foundry_core',
        ),
    ...
    test('ignores tags that do not match the full template', () {
      expect(
        parseVersionFromTag(
          tag: 'foundry_core/extra/0.0.1-dev.2',
          format: '{name}/{version}',
          name: 'foundry_core',
        ),
        isNull,
    ...
    test('ignores tags with unparseable version segments', () {
      expect(
        parseVersionFromTag(
          tag: 'foundry_core/not-a-version',
          format: '{name}/{version}',
          name: 'foundry_core',
        ),
        isNull,
  ...
  group('resolveLatestTag', () {
    test('returns highest semver among matching annotated tags', () {
      ...
      _gitCreateAnnotatedTag(tempRoot, 'foundry_core/0.0.1-dev.1');
      _gitCreateAnnotatedTag(tempRoot, 'foundry_core/0.0.1-dev.3');
      _gitCreateAnnotatedTag(tempRoot, 'foundry_core/0.0.1-dev.2');
      _gitCreateAnnotatedTag(tempRoot, 'foundry_cli/0.0.1-dev.9');
      _gitCreateAnnotatedTag(tempRoot, 'foundry_core/not-a-version');
      _gitCreateAnnotatedTag(tempRoot, 'foundry_core/0.0.1-dev.2-extra');

      final result = resolveLatestTag(
        gitRoot: tempRoot,
        tagFormat: '{name}/{version}',
        packageName: 'foundry_core',
      );

      expect(result.errorMessage, isNull);
      expect(result.tag, 'foundry_core/0.0.1-dev.3');
    ...
    test('returns null tag when no matching tags exist', () {
      _initGitRepoWithCommit(tempRoot);
      _gitCreateAnnotatedTag(tempRoot, 'foundry_cli/0.0.1-dev.1');
  ...
  group('parseConventionalCommitSubject', () {
    test('parses scoped commit with single scope', () {
      final commit = parseConventionalCommitSubject(
        'fix(foundry_cli): resolve relative paths from project root',
      );

      expect(commit, isNotNull);
      expect(commit!.type, 'fix');
      expect(commit.scopes, ['foundry_cli']);
      expect(
        commit.description,
    ...
    test('parses scoped commit with multiple scopes', () {
      final commit = parseConventionalCommitSubject(
        'feat(foundry_cli,clay_vsc_extension): wire preview command',
      );

      expect(commit, isNotNull);
      expect(commit!.type, 'feat');
      expect(commit.scopes, ['foundry_cli', 'clay_vsc_extension']);
      expect(commit.description, 'wire preview command');
    });

    test('parses breaking change indicator in header', () {
      final commit = parseConventionalCommitSubject(
        'feat(foundry_cli)!: drop legacy preview flag',
      );
    ...
    test('normalizes commit type to lowercase', () {
      final commit = parseConventionalCommitSubject(
        'FEAT(foundry_cli): add preview command',
      );
    ...
    test('returns null when description is missing', () {
      expect(parseConventionalCommitSubject('feat(foundry_cli):'), isNull);
    });
  });
  ...
  group('filterConventionalCommits', () {
    test('includes multi-scope commits when package name matches', () {
      final filtered = filterConventionalCommits(
        subjects: const [
          'feat(foundry_cli,clay_vsc_extension): add preview',
          'feat(foundry_core): add validation',
        ],
        packageName: 'foundry_cli',
        allowedTypes: const {'feat', 'fix'},
      );

      expect(filtered, hasLength(1));
      expect(filtered.single.type, 'feat');
      expect(filtered.single.scopes, ['foundry_cli', 'clay_vsc_extension']);
    });
    ...
    test('excludes unscoped and wrong-scope commits', () {
      ...
      expect(filtered, hasLength(1));
      expect(filtered.single.subject, 'fix(foundry_cli): resolve paths');
    });

    test('excludes commits whose type is not allowed', () {
      final filtered = filterConventionalCommits(
        subjects: const [
          'chore(foundry_cli): release 0.0.1-dev.2',
          'feat(foundry_cli): add preview',
        ],
        packageName: 'foundry_cli',
        allowedTypes: const {
          'feat',
  ...
  group('hasBreakingChange', () {
    test('detects breaking indicator in header', () {
      const commit = ConventionalCommit(
        ...
        subject: 'feat(foundry_cli)!: drop legacy flag',
    ...
    test('detects BREAKING CHANGE footer in body', () {
      const commit = ConventionalCommit(
        ...
        subject: 'feat(foundry_cli): reshape preview API',
      ...
      final result = computeNextVersion(
        ...
        commits: [
          const ConventionalCommit(
            ...
            subject: 'fix(foundry_cli): resolve paths',
            isBreakingChange: false,
          ),
        ],
      );
  ...
  group('checkReleaseSafetyGate', () {
    test('passes when pubspec version matches latest tag', () {
      final result = checkReleaseSafetyGate(
        ...
        latestTag: 'foundry_core/0.0.1-dev.2',
    ...
    test('fails when pubspec is ahead of latest tag', () {
      ...
      expect(result.errorMessage, contains('foundry_core/0.0.1-dev.3'));
    });
    ...
    test('fails when no release tags exist', () {
      ...
      expect(result.errorMessage, contains('foundry_core/0.0.1-dev.2'));
    });
...
ProcessResult _runPrepareReleaseCli({
  ...
  return Process.runSync(
    ...
    workingDirectory: foundryRepoRoot.path,
  );
}

Directory _resolveFoundryRepoRoot() {
  ...
  fail(
    'Could not resolve foundry repo root (cwd: ${Directory.current.path})',
  );
}
